...
- **BrandServiceItem** - New selectable brand/service chip component with state/defaults API and optional icon support
- **Async icon model for BrandServiceItem** - Added `BrandServiceItemIconModel.Async` with `loading`, `error`, and `placeholder` customization hooks
- **SupportButton** - New outlined support action button component (`SupportButton`, `SupportButtonState`, `SupportButtonDefaults`)
- **Example app target** - Added `:example` Android host app for validating components before publishing

### Changed

- **BrandServiceItem icon tint** - Added `iconTint` parameter to control tint for local/async/fallback icon painters
- **CommonMain previews setup** - Added `androidRuntimeClasspath(compose-ui-tooling)` for AGP 9 + Android KMP library preview support

### Fixed

- **Example build stability** - Added project `gradle.properties` JVM memory defaults to prevent D8 OOM during `:example:assembleDebug`

## [2.2.4] - 2026-02-16

### Fixed

- **Button loading indicators** - `PrimaryButton` and `SecondaryButton` now pass their current container color to `NativeLoadingIndicator` background to keep spinner background aligned with the placement surface across platforms

## [2.2.3] - 2026-02-16

### Fixed

- **Sheet insets** - Removed duplicate bottom navigation inset application in sheet content where `Sheet`/`AnimatedSheet` already provides `WindowInsets.navigationBars`
- **Affected components** - `ActionSheet`, `ConfirmationSheet`, `DatePickerSheet`

## [2.2.2] - 2026-02-11

### Added

- **ActionPickerItem** - Added `isDestructive` parameter (default `false`) to show icon in red for destructive actions like delete
- **ActionPickerDefaults.colors()** - Added `destructiveItemIcon` color parameter (default `System.color.iconRed`)
