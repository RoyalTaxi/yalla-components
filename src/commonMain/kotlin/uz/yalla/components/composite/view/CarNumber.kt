...
object CarNumberDefaults {
    ...
    internal const val BaseHeight: Float = 24f
    ...
    @Composable
    fun style(
        number: TextStyle = System.font.custom.carNumber
    ) = CarNumberStyle(
        number = number
    )
...
fun CarNumber(
    ...
    val scale = remember(dimens.height) { dimens.height.value / CarNumberDefaults.BaseHeight }
    ...
    Card(
        ...
        Row(
            ...
            Box(
                modifier = Modifier.size(dimens.dotSize.scaled()).background(
                    shape = CircleShape,
                    color = colors.text
                )
            )
